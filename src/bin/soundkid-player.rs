...
fn main() {
    let matches = App::new("soundkid-player")
        ...
        .arg(
            Arg::with_name("spotify_username")
                .required(true)
                .index(1)
                .help("The spotify username"),
        )
        .arg(
            Arg::with_name("spotify_password")
                .required(true)
                .index(2)
                .help("The spotify password"),
        )
        .arg(
            Arg::with_name("spotify_id")
                .required(true)
                .index(3)
...
mod player {
    ...
    impl SpotifyPlayer {
        pub fn new(username: String, password: String) -> Self {
            ...
            let (p, _) = Player::new(player_config, s.clone(), None, move || (backend)(None));
        ...
        pub fn play(&mut self, uri: String) {
            ...
            match spotify_uri_type {
                SpotifyUriType::Album => {
                    tracks = self
                        .core
                        .run(Album::get(&self.session.clone(), spotify_id))
                        .unwrap()
                        .tracks
                }
                SpotifyUriType::Playlist => {
                    tracks = self
                        .core
                        .run(Playlist::get(&self.session.clone(), spotify_id))
                        .unwrap()
                        .tracks
                }
